...
# db.select([MsgMESSAGE.subject], where, options)

# map(lambda m: map(lambda x: x.user, m.sent_to_users), get_all_messages_for_user(1))
# map(lambda m: m.recipients, get_all_messages_for_user(1))
# map(lambda m: m., get_all_messages_for_user(1))

# [[User(1), User(2)], [User(1)]]


# map(lambda x: x.status, UserMsgMESSAGE.query.filter_by(user_id=uid))

# db.select([UserMsgMESSAGE.status], UserMsgMESSAGE.user_id==uid)
